...
#   - VOICEVOX_SPEAKER_POOL: VOICEVOX用話者IDリスト
#   - AIVISSPEECH_SPEAKER_POOL: AivisSpeech用話者IDリスト
...
# VOICEVOX話者設定
VOICEVOX_DEFAULT_SPEAKER=47   # ナースロボ＿タイプT（ノーマル）
VOICEVOX_SPEAKER_POOL=(
    47  # ナースロボ＿タイプT（ノーマル）
    3   # ずんだもん（ノーマル）
    0   # 四国めたん（ノーマル）
    8   # 春日部つむぎ（ノーマル）
...
# AivisSpeech話者設定
AIVISSPEECH_DEFAULT_SPEAKER=888753760  # Anneli Normal
AIVISSPEECH_SPEAKER_POOL=(
    ...
    606865152  # fumifumi ノーマル
    933744512  # peach ノーマル
    706073888  # white ノーマル
    1431611904 # まい ノーマル
    376644064  # 桜音 ノーマル
    1325133120 # 花音 ノーマル
)
...
if [[ -n "$config_command" ]]; then
    case "$config_command" in
        ...
        "show_config")
            # 現在の設定を表示
            echo "📊 現在の設定:" >&2
            echo "VOICEVOX話者プール: $(grep -A 10 'VOICEVOX_SPEAKER_POOL=(' "$0" | grep -o '[0-9]\+' | tr '\n' ', ' | sed 's/,$//')" >&2
            echo "AivisSpeech話者プール: $(grep -A 5 'AIVISSPEECH_SPEAKER_POOL=(' "$0" | grep -o '[0-9]\+' | tr '\n' ', ' | sed 's/,$//')" >&2
            grep -E "^(ENGINE_PRIORITY|READING_MODE|MAX_CHARS|MAX_LINES)=" "$0" >&2
            exit 0 ;;
    esac
fi
...
if curl -s "http://127.0.0.1:50021/version" >/dev/null 2>&1; then
    ...
    echo "🔧 VOICEVOX ENGINE が利用可能です" >> /tmp/voice_notification_debug.log
fi

if curl -s "http://127.0.0.1:10101/speakers" >/dev/null 2>&1; then
    aivisspeech_available=true
    echo "🔧 AivisSpeech が利用可能です" >> /tmp/voice_notification_debug.log
fi
...
echo "🔧 使用する音声エンジン: $selected_engine" >> /tmp/voice_notification_debug.log

# エンジンに応じて話者を選択
if [[ "$selected_engine" == "voicevox" ]]; then
    speaker=${VOICEVOX_SPEAKER_POOL[$RANDOM % ${#VOICEVOX_SPEAKER_POOL[@]}]}
    echo "🔧 VOICEVOX ランダム話者: $speaker" >> /tmp/voice_notification_debug.log
else
    speaker=${AIVISSPEECH_SPEAKER_POOL[$RANDOM % ${#AIVISSPEECH_SPEAKER_POOL[@]}]}
    echo "🔧 AivisSpeech ランダム話者: $speaker" >> /tmp/voice_notification_debug.log
fi
...
if [[ "$selected_engine" == "voicevox" ]]; then
    ...
    echo "🔧 VOICEVOX ENGINEで音声合成を実行" >> /tmp/voice_notification_debug.log
    ...
    echo "🔧 AivisSpeechで音声合成を実行" >> /tmp/voice_notification_debug.log
    
    # AivisSpeech API呼び出し（VOICEVOX互換形式）
    query_json=$(mktemp)
    if ! curl -s -X POST "http://127.0.0.1:10101/audio_query?speaker=$speaker" \
         --get --data-urlencode "text=$text" -o "$query_json"; then
    ...
    if ! curl -s -H 'Content-Type: application/json' \
         -d @"$query_json" \
         "http://127.0.0.1:10101/synthesis?speaker=$speaker" -o "$wave_file"; then
...
if command -v afplay >/dev/null 2>&1; then
    echo "🔊 音声を再生中... (Engine: $selected_engine, Speaker: $speaker)" >&2
