#!/usr/bin/env bash
# ========================================
# Claude Code Stop Hook - 簡易音声読み上げ
# ========================================
# 
# 概要: Claude Codeの Stop フックで呼び出され、
#       固定メッセージを簡易に読み上げるスクリプト
#
# 依存関係:
...
#   - VOICEVOX_SPEAKER: VOICEVOX話者ID
#   - AIVISSPEECH_SPEAKER: AivisSpeech話者ID
#
# 使用方法:
#   - Claude Code の Stop フックとして自動実行
#
# ログ出力: /tmp/voice_stop_debug.log
# ========================================
...
# AivisSpeech話者設定（全話者対応）
AIVISSPEECH_SPEAKER=888753760  # Anneli ノーマル（デフォルト）
# 利用可能な話者ID:
...
if curl -s "http://127.0.0.1:50021/version" >/dev/null 2>&1; then
    ...
    echo "🔧 VOICEVOX ENGINE が利用可能です" >> /tmp/voice_stop_debug.log
fi

if curl -s "http://127.0.0.1:10101/speakers" >/dev/null 2>&1; then
    aivisspeech_available=true
    echo "🔧 AivisSpeech が利用可能です" >> /tmp/voice_stop_debug.log
fi
...
if [[ -z "$selected_engine" ]]; then
    ...
    echo "VOICEVOX ENGINE または AivisSpeech を起動してください" >&2
    exit 0
fi

echo "🔧 使用する音声エンジン: $selected_engine, 話者: $speaker" >> /tmp/voice_stop_debug.log
